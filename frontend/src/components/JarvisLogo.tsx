...
/**
 * The Jarvis mark: the assistant, arm around a rising chart. Drawn inline rather than loaded
 * from /jarvis-mark.svg so it is there on the first paint and costs no request.
 *
 * The robot's shell is near-white, so on a light page it needs something behind it: `tile`
 * (the default) wraps it in the same navy squircle the app icons use. Pass `tile={false}`
 * only where the surface is already dark.
 */
export default function JarvisLogo({
  ...
  // Two Jarvis logos on one page (sidebar + mobile header) would otherwise share gradient ids.
  ...
  const glyph = (
    <>
      ...
      <g>
        ...
        <path
          d="M78 216 C78 182 93 164 112 164 C131 164 146 182 146 216 Z"
          fill={`url(#${id("shell")})`}
        />
