...
/**
 * Has a reminder occurrence been paid? A debit of about the reminder's amount (±2%) dated within
 * 5 days before to 2 days after the due date counts as the payment. Transfers and card bill
 * settlements are ignored. Without an amount only the date can be judged.
 */
export function reminderStatus(
  ...
  if (amount && amount > 0) {
    ...
    if (match) return { state: "paid", txn: match };
  }
